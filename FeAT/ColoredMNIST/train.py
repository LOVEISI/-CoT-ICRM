...
def clove_train(mlp, topmlp, steps, envs, test_envs, lossf, \
    ...
    def mmce_penalty(logits, y, kernel='laplacian'):
        ...
        confidence = torch.ones(len(y_hat)) 
...
def lff_train(mlp, topmlp, steps, envs, test_envs, lossf, \
    ...
    for step in range(penalty_anneal_iters, steps):
        ...
        loss_d_update = criterion(logit_d, y) * loss_weight 
...
def ifeat_train(mlp, topmlp, steps, envs, test_envs, lossf, \
    ...
    y = torch.cat([envs[i]['labels'] for i in range(len(envs))])
    print(x.shape)
    x = x.mean(dim=1, keepdim=True)  # 如果是RGB三通道图片，取平均值转换为单通道
    print("After mean-------------------------------------------------")
    print(x.shape)
    logs = []
    classifiers = []
    ...
    for round_i in range(rounds):
        ...
        topmlp = TopMLP(hidden_dim = hidden_dim, n_top_layers=1, n_targets=n_targets) 
        ...
        for step  in range(steps_per_round):
            ...
            if False and len(classifiers) > 0:
                ...
                print(x.shape)
                overall_feats = mlp(x)
            print("下面是overall_feats的shape")
            print(overall_feats.shape)
            if overall_feats.numel() % 128 == 0:
                overall_feats = overall_feats.view(-1, 128)
            else:
                print("不能使用 view(-1, 128)，因为元素总数不能被 128 整除")
            ...
            if step % eval_steps == 0:
                ...
                if train_acc.item() > best_train_acc:
