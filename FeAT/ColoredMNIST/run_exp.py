...
from mydatasets import coloredmnist,femnist

from models import MLP, TopMLP,ContextNet
...
def main(flags):
    ...
    hparams = {
        # Featurizer settings for ContextNet
        'num_features': 1,         # Number of output channels (1 for grayscale images, adjust as needed)
        'kernel_size': 5,          # Kernel size used in Conv2d layers
        'hidden_dim': 128,         # Hidden dimension in the Conv2d layers, as seen in your `ContextNet` definition
        'is_transformer': False,   # Whether using transformer, based on your architecture choice
        'context_length': 784,     # Depending on your input, this may represent the flattened input size or a transformer context length
        'freeze_bn': True,         # To freeze batch normalization if needed
        'mlp_width': 500,          # Width of MLP layers (can be used in `MLP` if relevant)
        'mlp_depth': 5,            # Depth of MLP layers (if used)
        'activation': 'relu',      # Activation function
        'mlp_dropout': 0.5,        # Dropout for MLP
        'mlp_bn': 1,               # Batch normalization in MLP
        'mlp_ln': 0,               # Layer normalization in MLP
        'resnet_dropout': 0.5,     # Dropout for ResNet (if using)
        'densenet_dropout': 0.5,   # Dropout for DenseNet (if using)
        'resnet18': True,          # Flag to use ResNet18 (if needed)
        'resnet50': False,         # Flag to use ResNet50 (if needed)
        'densenet121': False,      # Flag to use DenseNet121 (if needed)
    }
    ...
    for restart in range(flags.n_restarts):
        ...
        elif flags.dataset == 'femnist':
            envs, test_envs = femnist(flags.femnist_data_dir, int_target=int_target)
        else:
            raise NotImplementedError
        

        # mlp = MLP(hidden_dim = flags.hidden_dim, input_dim=input_dim) 
        # topmlp = TopMLP(hidden_dim = flags.hidden_dim, n_top_layers=flags.n_top_layers, \
        #     n_targets=n_targets, fishr= flags.methods=='fishr') 
        
                # Create the ContextNet and TopMLP models
        context_net = ContextNet(input_dim=1, hparams=hparams)  # You can adjust input_dim and hparams as needed
        topmlp = TopMLP(hidden_dim=hparams['hidden_dim'], n_top_layers=flags.n_top_layers, 
                        n_targets=1, fishr=False)

        print(context_net, topmlp)
        ...
        if flags.load_model_dir is not None and os.path.exists(flags.load_model_dir):
            ...
            context_net.load_state_dict(state)
        ...
        params = [context_net, topmlp, flags.steps, train_envs, test_envs,lossf,\
        ...
        if flags.methods in ['vrex', 'iga','irm','fishr','gm','lff','erm','dro','feat','irmx','ibirm']:
            res = train_func(*params)
        elif flags.methods == 'ifeat':
            hparams = {
                'rounds': flags.rounds,
                'steps_per_round': flags.steps_per_round,
                'stage2_methods': flags.stage2_methods
            }
            res = train_func(*params, hparams=hparams)
        ...
        if flags.save_dir is not None:
            ...
            print(context_net)
            print("TopMLP Model Structure:")
            print(topmlp)
            state = context_net.state_dict()
...
if __name__ == "__main__":
    ...
    parser.add_argument('--femnist_data_dir', type=str, default='G:/feta/ICRM/ICRM/data/femnist-data',
                        help='FEMNIST 数据集根目录，有 train/ 和 test/ 子目录')
