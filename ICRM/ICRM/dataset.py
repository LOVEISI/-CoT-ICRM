...
DATASETS = ['FEMNIST', 'RareGroupRotatedMNIST', 'TinyImagenet', 'WILDSCamelyon','ColouredMNIST']
...
class FEMNIST(MultipleDomainDataset):
    N_WORKERS = 0         #数据加载时的线程数为 0
    def __init__(self, root, test_envs, hparams):
        self.ctxt = hparams.get('context_length', 1) #当前上下文长度，用于多步输入或 Transformer 类模型
        self.seed = hparams.get('trial_seed',0)
        self.datasets = []    #用于存储多个客户端的训练集
        ...
        #clients: 客户端（即“用户”）的 ID 列表。
        #self.data: 是一个字典，键是客户端 ID，值是该客户端的数据
        assert len(clients) == len(set(clients)), 'duplicate users' #确保客户端列表中没有重复的用户 ID
        self.environments = list(range(len(clients)))
        
        # training data
        self.domain_counts = {} #用于记录每个客户端有多少个样本
        self.train_cache_x, self.train_cache_y = [], [] #用于存储 上下文样本
        for i, client in enumerate(clients):#遍历所有客户端，clients 是用户/客户端 ID 的列表（例如 ['f0000_01', 'f0001_02', ...]
            x, y, domain_count, cache_x, cache_y = self.make_dataset(self.data, client)
            self.domain_counts[client] = domain_count #记录当前客户端的样本数量
        ...
        datasets = [self.make_dataset(v_data, v_client, mode='val') for v_client in val_clients] #datasets 是一个列表，包含了多个客户端的数据，每个客户端的数据是一个元组：(x, y, len(client_X), rand_x, rand_y))
...
class RareGroupRotatedMNIST(MultipleEnvironmentMNIST):
    ...
    def __init__(self, root, test_envs, hparams):
        ...
        print("下满是--------------------")
        print(root)
        tr = MNIST(root, train=True, download=False)
        te = MNIST(root, train=False, download=False)
...
class OnFlyEnvironment:
    ...
    def _process_dict(self, data):
        ...
        return x, y
   #基本的数学公式讲清楚，都有论据来支持,讲这两篇paper，claim
   #code:换数据集，ppt
...
# class ColouredMNIST(MultipleDomainDataset):
#     """
#     Coloured MNIST with context cache.
#     Each environment has a different P(correct color | even/odd).
#     """
#     class ColorConfig:
#         # 在这里定义每个环境的“正确着色”概率
#         env_probs = torch.tensor([0.9, 0.2, 0.25], dtype=torch.float32)
...
#         # 1) 读入 MNIST 并拼在一起
#         mnist_tr = MNIST(root, train=True,  download=True)
#         mnist_te = MNIST(root, train=False, download=True)
#         images = torch.cat([mnist_tr.data, mnist_te.data], dim=0).numpy()
#         labels = torch.cat([mnist_tr.targets, mnist_te.targets], dim=0).numpy()
...
#         # 3) 对每个环境按概率 p 抽样并上色
#         for p in self.ColorConfig.env_probs:
#             x_tr, y_tr, tr_cx, tr_cy = self._make_env(images, labels, float(p), mode='train')
#             self.datasets.append(TensorDataset(x_tr, y_tr))
#             self.train_cache_x.append(tr_cx)
#             self.train_cache_y.append(tr_cy)
#             self.domain_counts.append(len(y_tr))

#             x_va, y_va, va_cx, va_cy = self._make_env(images, labels, float(p), mode='val')
#             self.validation.append(TensorDataset(x_va, y_va))
#             self.valid_cache_x.append(va_cx)
#             self.valid_cache_y.append(va_cy)

#             x_te, y_te, te_cx, te_cy = self._make_env(images, labels, float(p), mode='test')
#             self.holdout_test.append(TensorDataset(x_te, y_te))
#             self.test_cache_x.append(te_cx)
#             self.test_cache_y.append(te_cy)

#         # 4) 打印信息
#         print(f"=> n training domains: {len(self.datasets)}; sizes = {self.domain_counts}")
#         print(f"=> n validation domains: {len(self.validation)}")
#         print(f"=> n testing domains:    {len(self.holdout_test)}\n")
...
#     def _make_env(self, images, labels, correct_color_prob, mode='train'):
#         """
#         images: np.ndarray of shape [N, 28, 28]
#         labels: np.ndarray of shape [N]
#         correct_color_prob: float in [0,1]
#         mode: 'train' | 'val' | 'test'
#         """
#         N = images.shape[0]
#         # 抽取 n_keep = p * N 张样本
#         n_keep = int(correct_color_prob * N)
#         idx = np.random.default_rng(self.seed).choice(N, size=n_keep, replace=False)

#         # 上色后的数据存放在 xs, ys
#         xs = torch.zeros(n_keep, 3, 28, 28, dtype=torch.float32)
#         # ys = torch.tensor(labels[idx] % 2, dtype=torch.long)  # 只取 even/odd
#                 # 标签定义：小于5为正类（1），否则为负类（0）
#         ys = (torch.tensor(labels[idx], dtype=torch.float32) < 5).float()
#         # 加入标签扰动：25%概率翻转标签
#         flip = torch.bernoulli(torch.full_like(ys, 0.25))
#         ys = torch.abs(ys - flip).long()

#         for i, orig_i in enumerate(idx):
#             img = images[orig_i] / 255.0  # 归一化
#             lbl = labels[orig_i] % 2
#             # 根据 correct_color_prob 决定是否颠倒
#             if random.random() < correct_color_prob:
#                 color = 'red' if lbl == 0 else 'green'
#             else:
#                 color = 'green' if lbl == 0 else 'red'
#             # 填充 RGB 通道
#             if color == 'red':
#                 r, g, b = img, np.zeros_like(img), np.zeros_like(img)
#             else:
#                 r, g, b = np.zeros_like(img), img, np.zeros_like(img)
#             xs[i,0], xs[i,1], xs[i,2] = torch.from_numpy(r), torch.from_numpy(g), torch.from_numpy(b)

#         # 生成上下文 cache
#         if self.ctxt > 1:
#             rng = torch.Generator().manual_seed(self.seed)
#             ctx_idx = torch.randint(0, n_keep, (self.ctxt-1,), generator=rng)
#             cx = xs[ctx_idx].unsqueeze(0)   # [1, ctxt-1, 3,28,28]
#             cy = ys[ctx_idx].unsqueeze(0)   # [1, ctxt-1]
#             # eval 模式时，保持 shape=[1,ctxt-1,...]
#             if mode != 'train':
#                 xs = xs.unsqueeze(1)
#                 ys = ys.unsqueeze(1)
#         else:
...
class ColouredMNIST(MultipleDomainDataset):
    ...
    def __init__(self, root, test_envs, hparams):
        ...
        for p in self.ColorConfig.env_probs:
            # train env
            x_tr, y_tr, tr_cx, tr_cy = self._make_env(images[train_idx], labels[train_idx], float(p), mode='train')
            self.datasets.append(TensorDataset(x_tr, y_tr))
            self.train_cache_x.append(tr_cx)
            self.train_cache_y.append(tr_cy)
            self.domain_counts.append(len(y_tr))
            ...
            # val env
            x_va, y_va, va_cx, va_cy = self._make_env(images[val_idx], labels[val_idx], float(p), mode='val')
            self.validation.append(TensorDataset(x_va, y_va))
            self.valid_cache_x.append(va_cx)
            self.valid_cache_y.append(va_cy)

            # test env
            x_te, y_te, te_cx, te_cy = self._make_env(images[test_idx], labels[test_idx], float(p), mode='test')
            self.holdout_test.append(TensorDataset(x_te, y_te))
            self.test_cache_x.append(te_cx)
            self.test_cache_y.append(te_cy)
    ...
    def _make_env(self, images, labels, correct_color_prob, mode='train'):
        ...
        if self.ctxt > 1:
            rng = torch.Generator().manual_seed(self.seed)
            ctx_idx = torch.randint(0, n_keep, (self.ctxt-1,), generator=rng)
            cx = xs[ctx_idx].unsqueeze(0)
            cy = ys[ctx_idx].unsqueeze(0)
            if mode != 'train':
                xs = xs.unsqueeze(1)
                ys = ys.unsqueeze(1)
        else:
